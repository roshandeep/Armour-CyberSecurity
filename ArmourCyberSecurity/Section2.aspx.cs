...
namespace ArmourCyberSecurity
{
    public partial class Section2 : System.Web.UI.Page
    {
        ...
        {
            /**
             * Load the Level2 questions for Section 2 Part 1 "Foundation - Privacy by Design"
        ...
        private void SaveAnswers(DataTable dt)
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                if (row["question_type"].ToString() == "Privacy by Design" && row["section"].ToString() == "2")
                {
        ...
        public void SetDeafaults(DataTable dt, string userId, string question_type, int stagesCompleted)
        {
            ...
            if (count == 0)
            {
                foreach (DataRow row in dt.Rows)
                {
                    if (row["question_type"].ToString() == question_type && row["section"].ToString() == "2")
                    {
                        quesId = Convert.ToInt32(row["question_id"].ToString());
                        answer = "--SELECT--";
                        answerWt = "-1";
